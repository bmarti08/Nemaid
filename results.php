...
<div id="results"><table>
	<?php 
		if($_SESSION['res_type'] == "composite") {
			echo "You have chosen the algorithms using the Composites descriptions with the <b>".$_SESSION['algoVersion']."</b> version. ";
			}
		elseif ($_SESSION['res_type'] == "mixed") {
			echo "You have chosen the algorithms using the Composites and the Original descriptions with the <b>".$_SESSION['algoVersion']."</b> version. ";
		}
		elseif ($_SESSION['res_type'] == "originale") {
			echo "You have chosen the algorithms using the Original descriptions only with the <b>".$_SESSION['algoVersion']."</b> version. ";
		}
		else {
			echo "You have chosen the algorithms using all the Original descriptions and later descriptions treated individually (no composite description) with the <b>".$_SESSION['algoVersion']."</b> version. ";
		}
		
		if (isset($_POST['validity'])){ 
			echo '</br>'."<b>Invalid descriptions</b> are included.".'</br>';
		} else {
			echo '</br>'."<b>Invalid descriptions</b> are not included.".'</br>';
		}
	?>
	...
	<?php
	$count = 0;
	foreach($results as $spe => $res) {
		if ($res['nb_char_used'] != 0) {
			...
			if(isset($res['details']['qt'])) {
				echo '<tr>
					<th colspan="2">Quantitative and 1-state qualitative characters</th>
					<th>Sample</th>
					<th>Species</th>
					<th>Score</th>
					<th>Weight</th>
			...
			if(isset($res['details']['ql'])) { // if the character is qualitative
				echo '<tr>
						<th colspan="2">Qualitative characters (2-states and more)</th>
						<th>Sample</th>
						<th>Species</th>
						<th>State score</th>
					...
					echo '<tr>
							...
							if($nb_states == 1) {
								...
								if (isset($details['char_score'])) {
									echo '<tr>
									<td></td>
									<td><b>'.substr($character,0,-2).'</b></td>
									<td colspan = "3"></td>
