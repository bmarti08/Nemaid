...
<p class="alinea1"> 2 - Open the tab "Your samples", then:</p>
...
<span class="alinea1">In the tab <b>Perform a comparison</b>, Ã¢â‚¬Â¦??
...
<!----------------------------------------------------------------------->
	<tr>
	<td class="no_border"><button onClick = "showHideDetails('<?php echo $count++; ?>')">+</button>What computation formulae are used in version 3.3?<br /></td>
	</tr>
	<tr class="details">
		<td class="no_border">
			<table>
	...
	<tr class="details">
		<td class="no_border">
			<table>
				<tr >
					<td>
						Nemaid integrates the variability observed for some characters among the species of any given genus. For example, in Helicotylenchus, it has been observed that the body length can vary by up to 150 Ã‚Âµm when the progeny of a single parthenogenetic female are raised on different hosts plants (<a href="http://genisys.prd.fr/Fortuner_1984c.pdf">Fortuner, 1984</a>). In the Nemaid formulae, the specific variability is represented by corrections factors and a difference in measurements or percentages between an unknown sample and a species is accepted as indicating a dissimilarity only in so far that such a difference is larger than the correction factor of the corresponding character. <br/> If you donÃ¢â‚¬â„¢t agree with the default values displayed in the tab Ã¢â‚¬Å“Your samplesÃ¢â‚¬ï¿½, you are free to modify them.
					<br />
					</td>
				</tr>
			</table>
...
If you donÃ¢â‚¬â„¢t agree with the default values displayed in the tab Ã¢â‚¬Å“Your samplesÃ¢â‚¬ï¿½, you are free to modify them. You can also chose to set all weights equal to 1, in which case the characters will no longer be weighted.
					<br />
					</td>
				</tr>
			</table>
	...
	<tr class="details">
		<td class="no_border">
			<table>
				<tr >
					<td>
						A character range is the difference between the maximum and minimum values found in the data table for the selected genus. For example, in Helicotylenchus, the largest species (<i>H. coomansi</i>) has a body length of 1235 Ã‚Âµm while the smallest species (<i>H. minutus</i>) is 400 Ã‚Âµm long. In that genus, the range for body length is 1235-400 = 835 Ã‚Âµm. Stylets are far shorter than the whole body and the range for stylet length is only 27 Ã‚Âµm. It is even smaller (2) for ratio <b>cÃ¢â‚¬â„¢</b>. Ranges are included in the Nemaid formulae in order to compare differences in characters with absolute values so widely different.</br>
